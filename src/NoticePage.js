...
const NoticePage = () => {
    ...
    try {
      ...
      const res = await axios.get(
        process.env.REACT_APP_SERVER_URL +
          `/api/request/list/schedule/member/${localStorage.getItem("userId")}`,
        config
      );
  ...
  const updateScheduleRequest = async (id, status) => {
    ...
    try {
      ...
      const res = await axios.put(
        process.env.REACT_APP_SERVER_URL + "/api/request/update/schedule",
        {
          requestId: id,
          status: status,
        },
  ...
  const deleteScheduleRequest = async id => {
    ...
    try {
      ...
      const res = await axios.delete(
        process.env.REACT_APP_SERVER_URL + `/api/request/delete/schedule/${id}`,
        config
      );
  ...
  const updateFriendRequest = async (id, status) => {
    ...
    try {
      ...
      const res = await axios.put(
        process.env.REACT_APP_SERVER_URL + "/api/request/update/friend",
        {
          requestId: id,
          status: status,
        },
  ...
  const deleteFriendRequest = async id => {
    ...
    try {
      ...
      const res = await axios.delete(
        process.env.REACT_APP_SERVER_URL + `/api/request/delete/friend/${id}`,
        config
      );
  ...
  const updateGroupRequest = async (id, status) => {
    ...
    try {
      ...
      const res = await axios.put(
        process.env.REACT_APP_SERVER_URL + "/api/request/update/group-invite",
        {
          requestId: id,
          status: status,
        },
  ...
  const TabContent = () => (
    <Tabs activeKey={tab} onChange={key => setTab(key)}>
      <TabPane
        tab={<span style={{ fontSize: "20px", fontWeight: "bold" }}>일정</span>}
        key="일정">
        {schedules.map(schedule => (
          <Card
            ...
            <Card.Meta
              avatar={
                <Avatar
                  src={schedule.sender.profileImagePath || <UserOutlined />}
                />
              }
              title={
              ...
              {schedule.status === "PENDING" ? (
                <>
                  <Button
                    ...
                    onClick={() =>
                      handleScheduleApproval(schedule.requestedScheduleId)
                    }>
                    승인
                  </Button>
                  <Button
                    type="danger"
                    onClick={() =>
                      handleScheduleRejection(schedule.requestedScheduleId)
                    }>
                    거절
                  </Button>
                </>
                ...
                <Button
                  type="danger"
                  onClick={() =>
                    handleScheduleDeletion(schedule.requestedScheduleId)
                  }>
                  삭제
                </Button>
              )}
        ...
        {schedules.length === 0 && (
          <Card style={{ width: "100%", marginTop: 16 }}>
            <Card.Meta title={<span>현재 등록된 알림이 없어요!</span>} />
          </Card>
        )}
      </TabPane>
      <TabPane
        tab={<span style={{ fontSize: "20px", fontWeight: "bold" }}>친구</span>}
        key="친구">
        {friends.map(friend => (
          <Card
            ...
            <Card.Meta
              avatar={
                <Avatar
                  src={friend.sender.profileImagePath || <UserOutlined />}
                />
              }
              title={
                <span
              ...
              {friend.status === "PENDING" ? (
                <>
                  <Button
                    ...
                    style={{ marginRight: "8px" }}
                    onClick={() =>
                      handleFriendAcceptance(friend.friendRequestId)
                    }>
                    수락
                  </Button>
                  <Button
                    type="danger"
                    onClick={() =>
                      handleFriendRejection(friend.friendRequestId)
                    }>
                    거절
                  </Button>
                </>
                ...
                <Button
                  type="danger"
                  onClick={() => handleFriendDeletion(friend.friendRequestId)}>
                  삭제
                </Button>
              )}
            </div>
      ...
      <TabPane
        tab={<span style={{ fontSize: "20px", fontWeight: "bold" }}>그룹</span>}
        key="그룹">
        {groups.map(group => (
          <Card
            ...
            <Card.Meta
              avatar={
                <Avatar
                  src={group.sender.profileImagePath || <UserOutlined />}
                />
              }
              title={
                <span style={{ fontSize: "16px", fontWeight: "bold" }}>
              ...
              {group.status === "PENDING" ? (
                <>
                  <Button
                    type="primary"
                    style={{ marginRight: "8px" }}
                    onClick={() => handleGroupAcceptance(group.groupRequestId)}>
                    수락
                  </Button>
                  <Button
                    type="danger"
                    onClick={() => handleGroupRejection(group.groupRequestId)}>
                    거절
                  </Button>
                </>
              ) : (
                <Button
                  type="danger"
                  onClick={() => handleGroupDeletion(group.groupRequestId)}>
                  삭제
                </Button>
              )}
            </div>
